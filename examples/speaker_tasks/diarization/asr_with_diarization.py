...
parser.add_argument('--asr_based_vad', default=False, action='store_true', help="Wheter to use ASR-based VAD")
parser.add_argument('--generate_oracle_manifest', default=False, action='store_true', help="Wheter to generate and use oracle VAD")
...
if not sum(bool(c) for c in vad_choices) == 1:
    raise ValueError("Please either provide ONE and ONLY ONE method for VAD! \n \
            (1) external manifest external_vad_manifest or \n \
            (2) use pretrained_vad_model, or \n \
...
if args.asr_based_vad:
    ...
    if args.reference_rttmfile_list_path:
        ...
        # oracle_manifest = asr_diar_offline.write_VAD_rttm(asr_diar_offline.oracle_vad_dir, audio_file_list)
...
asr_diar_offline.run_diarization(
    audio_file_list, oracle_manifest, args.oracle_num_speakers, args.pretrained_speaker_model, args.pretrained_vad_model
)
